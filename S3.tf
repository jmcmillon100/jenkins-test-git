...
resource "aws_s3_bucket" "my_jenkins_bucket" {
  ...
  tags = {
    Name = "Jenkins Webhook Bucket"
  }
}

resource "aws_s3_bucket_public_access_block" "my_jenkins_bucket_block" {
  bucket = aws_s3_bucket.my_jenkins_bucket.id
...
resource "aws_s3_bucket_policy" "public_read" {
  ...
  depends_on = [aws_s3_bucket_public_access_block.my_jenkins_bucket_block]
    ...
    Statement = [
      {
        ...
        Principal = "*"
        Action    = ["s3:GetObject"]
        Resource  = "${aws_s3_bucket.my_jenkins_bucket.arn}/*"
      }
    ]
  })
}
